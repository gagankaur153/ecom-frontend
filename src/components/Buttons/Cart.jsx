...
const Cart = () => {
 ...
 const { fetchcarts, carts,quantityDecrease,dec,inc,remove,quantityIncrease,quantityRemove,deletecart} = useContext(Appcontext)
  ...
  return (
   <>
     <div className='min-h-screen flex text-white flex-col gap-4 p-4 mt-28'>
     {
      carts?.item?.length === 0 ? <>
     ...
     <>
    {
     ...
     <div className='m-5 md:flex flex-col gap-8'>
     {
            carts?.item.map((product)=>(
                <div key={product._id} className=' bg-zinc-900 rounded-md mb-4 p-4 md:flex items-center md:justify-around  '>
               <div className='flex flex-row gap-4 md:gap-8'>
             <div className=''>
             <img className='w-[80px] h-[70px] md:w-[100px] md:h-[100px] rounded-xl ' src={product?.image}/>
             </div>
               <div className='flex flex-col p-2 justify-center font-bold space-y-3'>
             <div>  <p className='md:text-3xl ' >Title: {product?.productid?.title}</p></div>
                <div className='flex flex-row gap-3 md:gap-5'>
                <p className=' md:text-xl  '>Price: {product.price}</p>
                  <p className='md:text-xl' >Quantity: {product?.quantity}</p>
                </div>
                 </div>
               </div>
                 ...
                 <div className=' flex justify-around md:gap-5 p-2'>
                  <button className='bg-yellow-500 hover:bg-yellow-700 md:text-2xl px-2  rounded text-black font-bold ' onClick={()=>decrease(product?.productid?._id)} >Qty--</button>
                  <button className='bg-sky-500 hover:bg-sky-700  md:text-2xl px-2 rounded text-black font-bold'onClick={()=>increase(product?.productid?._id)} >Qty++</button>
                  <button className='bg-red-500 hover:bg-red-700   md:text-2xl px-2 rounded text-white font-bold'onClick={()=>rremove(product?.productid?._id)} >Remove</button>
                 </div>
